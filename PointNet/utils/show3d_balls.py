...
if __name__ == '__main__':
    ...
    # pts点云数据读取，法一：
    f = open('../shapenetcore_partanno_segmentation_benchmark_v0/02691156/points/1a04e3eab45ca15dd86060f189eb133.pts',
             'r')
    data = f.readlines()
    f.close()
    ...
    for line in data[0:]:
        line = line.strip('\n')
        point = line.split(' ')
        x, y, z = [eval(i) for i in point[:3]]
        pts.append([x, y, z])
    res = np.random.randn(len(pts), len(pts[0]))
    for i in range(len(pts)):
        res[i] = pts[i]

    # pts点云数据读取，法二：
    point_set = np.loadtxt('../shapenetcore_partanno_segmentation_benchmark_v0/02691156/points/1a04e3eab45ca15dd86060f189eb133.pts').astype(np.float32)
